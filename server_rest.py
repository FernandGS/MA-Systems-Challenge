...
# Optional local saving of Unity payloads (controlled via env)
PAYLOAD_SAVE_ENABLED = os.environ.get('SIM_SAVE_PAYLOAD', '1') not in ('0', 'false', 'False')
PAYLOAD_SAVE_DIR = os.environ.get('SIM_SAVE_DIR', os.path.join(ROOT, 'saved_payloads'))
if PAYLOAD_SAVE_ENABLED:
    try:
        os.makedirs(PAYLOAD_SAVE_DIR, exist_ok=True)
    except Exception:
        PAYLOAD_SAVE_ENABLED = False
...
def _validate_and_fix_payload(city: City, sim: Simulation, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Best-effort validation and repair for the Unity payload to guard against malformed arrays.
    - Ensures each agent.start has 2 ints.
    - Ensures each pathObj entry has both x and y; fills from previous or start if missing.
    - Ensures each bin.pos has 2 ints; falls back to sim.bins by id index.
    - Recomputes agent distance after fixes.
    Returns the same payload dict (mutated) for chaining.
    """
    try:
        # Fix agents
        ...
        for a in agents:
            ...
            if not isinstance(st, list) or len(st) != 2:
                a["start"] = [int(round(city.depot[0])), int(round(city.depot[1]))]
            else:
                a["start"] = [int(st[0]), int(st[1])]
            # path entries
            path = a.get("pathObj") or []
            last_x, last_y = a["start"][0], a["start"][1]
            fixed_path = []
            for p in path:
                ...
                if x is None and y is None:
                    ...
                    y = last_y
                x, y = int(x), int(y)
                fixed_path.append({"x": x, "y": y})
                last_x, last_y = x, y
            a["pathObj"] = fixed_path
            # distance
            dist_sum = 0
            for i in range(1, len(fixed_path)):
                dx = fixed_path[i]["x"] - fixed_path[i-1]["x"]
                dy = fixed_path[i]["y"] - fixed_path[i-1]["y"]
                dist_sum += (dx*dx + dy*dy) ** 0.5
            a["distance"] = int(round(dist_sum))

        # Fix bins
        bins_out = payload.get("bins") or []
        for b in bins_out:
            pos = b.get("pos")
            if not isinstance(pos, list) or len(pos) != 2:
                # try fallback by id index
                try:
                    bid = int(b.get("id", 0))
                    sb = sim.bins[bid] if 0 <= bid < len(sim.bins) else None
                    if sb is not None:
                        b["pos"] = [int(round(sb.pos[0])), int(round(sb.pos[1]))]
                    else:
                        b["pos"] = [int(round(city.depot[0])), int(round(city.depot[1]))]
                except Exception:
                    b["pos"] = [int(round(city.depot[0])), int(round(city.depot[1]))]
        ...
        try:
            grid["depot"] = [int(round(city.depot[0])), int(round(city.depot[1]))]
            grid["width"] = int(grid.get("width", int(round(city.map_w if hasattr(city, 'map_w') else city.size[0])))) if hasattr(city, 'size') else int(grid.get("width", 0))
            grid["height"] = int(grid.get("height", int(round(city.map_h if hasattr(city, 'map_h') else city.size[1])))) if hasattr(city, 'size') else int(grid.get("height", 0))
            payload["grid"] = grid
        except Exception:
            pass
    except Exception:
        # Do not let validation crash the request
        pass
    return payload


def _select_best_weights(base_dir: str) -> str | None:
    """Scan weight directories for best/best.json; return dir with highest metric."""
    try:
        if not os.path.isdir(base_dir):
            return None
        candidates = []
        for root, dirs, files in os.walk(base_dir):
            if 'best.json' in files:
                try:
                    with open(os.path.join(root,'best.json'),'r') as f:
                        meta = json.load(f)
                    metric = float(meta.get('metric') or meta.get('reward') or meta.get('pickup_rate') or -1e9)
                    candidates.append((metric, root))
                except Exception:
                    continue
        if not candidates:
            return None
...
def _prepare_dqn_weights(cfg: Dict[str, Any]):
    if cfg.get('POLICY') != 'dqn':
        return
    auto = os.environ.get('DQN_BEST_AUTO','1') not in ('0','false','False')
    if not auto:
        return
    base_dir = cfg.get('DQN_WEIGHTS_DIR','dqn_weights')
    base_dir_abs = os.path.join(ROOT, base_dir) if not os.path.isabs(base_dir) else base_dir
    best_sub = _select_best_weights(base_dir_abs)
    if best_sub:
        # Use a live dir to continue training (copy only if different)
        live_dir = os.environ.get('DQN_LIVE_DIR', os.path.join(base_dir_abs,'live'))
        try:
            os.makedirs(live_dir, exist_ok=True)
            # copy *.pt if live empty or files differ
            import shutil
            existing = {f for f in os.listdir(live_dir) if f.endswith('.pt')}
            src_files = [f for f in os.listdir(best_sub) if f.endswith('.pt')]
            for f in src_files:
                if f not in existing:
                    shutil.copy2(os.path.join(best_sub,f), os.path.join(live_dir,f))
            # set config to live dir
            cfg['DQN_WEIGHTS_DIR'] = live_dir
        except Exception:
            # fallback: still point to best_sub
            cfg['DQN_WEIGHTS_DIR'] = best_sub
    else:
        # ensure base dir exists for fresh training
        try:
            os.makedirs(base_dir_abs, exist_ok=True)
        except Exception:
            pass
        cfg['DQN_WEIGHTS_DIR'] = base_dir_abs
...
def preview():
    ...
    city, sim = _run_simulation(cfg, steps, planner)
    return _validate_and_fix_payload(city, sim, _to_unity_payload(cfg, city, sim))
...
def set_defaults(req: DefaultsRequest):
    ...
    try:
        ...
        planner = DEFAULT_OVERRIDES.get('planner') or 'graph'
        city, sim = _run_simulation(cfg, steps, planner)
        payload = _validate_and_fix_payload(city, sim, _to_unity_payload(cfg, city, sim))
        from time import time as _now
...
def simulate(req: SimRequest, request: Request):
    ...
    city, sim = _run_simulation(cfg, steps, planner)
    payload = _to_unity_payload(cfg, city, sim)
    payload = _validate_and_fix_payload(city, sim, payload)
    if PAYLOAD_SAVE_ENABLED:
        try:
            ts = time.strftime('%Y%m%d_%H%M%S')
            fname = f"simulate_post_steps{steps}_agents{cfg.get('N_TRUCKS')}_bins{cfg.get('N_BINS')}_{ts}.json"
            with open(os.path.join(PAYLOAD_SAVE_DIR, fname), 'w', encoding='utf-8') as f:
...
def simulate_get(
    ...
    if PAYLOAD_SAVE_ENABLED:
        try:
            ts = time.strftime('%Y%m%d_%H%M%S')
            fname = f"simulate_get_steps{eff_steps}_agents{cfg.get('N_TRUCKS')}_bins{cfg.get('N_BINS')}_{ts}.json"
            with open(os.path.join(PAYLOAD_SAVE_DIR, fname), 'w', encoding='utf-8') as f:
                json.dump(payload, f, ensure_ascii=False, indent=2)
        except Exception:
            pass
...
@app.post("/debug_force_save")
def debug_force_save():
    """Debug endpoint: runs a tiny 5-step sim and forces a save, returning the attempted filename.
    Helps diagnose why saved_payloads remains empty for the user.
    """
    from copy import deepcopy as _dc
    cfg = _dc(CONFIG)
    _apply_overrides(cfg, num_agents=2, num_waste_locations=4)
    city, sim = _run_simulation(cfg, 5, 'graph')
    payload = _validate_and_fix_payload(city, sim, _to_unity_payload(cfg, city, sim))
    saved = False
    fname = None
    reason = None
    if PAYLOAD_SAVE_ENABLED:
        try:
            ts = time.strftime('%Y%m%d_%H%M%S')
            fname = f"debug_force_save_{ts}.json"
            full = os.path.join(PAYLOAD_SAVE_DIR, fname)
        ...
        reason = 'PAYLOAD_SAVE_ENABLED is False'
    return {
        'attempted': fname,
        'saved': saved,
        'save_dir': PAYLOAD_SAVE_DIR,
        'enabled': PAYLOAD_SAVE_ENABLED,
        'reason': reason
    }
