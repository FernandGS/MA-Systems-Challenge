...
class Truck:
    ...
    def apply_action(self, action: int, bins: List[BinObj], depot: Point, cfg: dict) -> float:
        ...
        # base wage cost (reward = -€ wage/time)
        ...
        if action == 0:  # move (follow planned route if any; else fallback straight)
            if self.route_pts:
                ...
                c = self._move_along_route(dt)
            # subtract movement € (energy + maintenance) from reward
            reward -= c

            # OPTIONAL: convenience auto-service to avoid brittle timing
            thr = cfg.get("APPROACH_RADIUS_M", 3.0)
            if self.assigned_bin:
                for b in bins:
                    if b.id == self.assigned_bin and dist(self.pos, b.pos) < thr and b.fill > 0:
        ...
        # OPTIONAL: mild shaping for approaching assigned bin
        if self.assigned_bin:
