...
def simulate(custom: Dict[str, Any] = Body(default=None)) -> Dict[str, Any]:
    ...
    # semilla siempre definida
    seed = params.get('seed')
    if seed is None:
        seed = random.randrange(1_000_000_000) # o un fijo. pej 42
    seed = int(seed)
    params['seed'] = seed #para que wasteModel.setup() pueda usarla


    model = WasteModel(params)
    #pasa seed (y opcionalmente steps) a agentpy.run para evitar bug
    model.run(seed=seed, steps=params.get('steps', None))
...
def simulate_get(
        ...
        """
        custom = {}
        if seed is not None: custom['seed'] = int(seed)
