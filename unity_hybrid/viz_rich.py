...
class RichViz:
    ...
    def _draw_static(self):
        ...
        # Optional lane centerlines (keep-right/left) for debugging
        if self.cfg.get("ENABLE_LANES", False):
            lane_off = float(self.cfg.get("LANE_OFFSET_M", 1.0))
            lane_segs_pos = []
            lane_segs_neg = []
            for r in self.city.roads:
                (x1, y1), (x2, y2) = r.polyline
                ...
                if L < 1e-6:
                    continue
                rx, ry = dy / L, -dx / L  # right-hand perpendicular
                lane_segs_pos.append([(x1 + rx * lane_off, y1 + ry * lane_off), (x2 + rx * lane_off, y2 + ry * lane_off)])
                lane_segs_neg.append([(x1 - rx * lane_off, y1 - ry * lane_off), (x2 - rx * lane_off, y2 - ry * lane_off)])
            lc_pos = LineCollection(lane_segs_pos, linewidths=0.8, alpha=0.35, colors=[(0.55, 0.55, 0.55, 1.0)], linestyles='--')
            lc_neg = LineCollection(lane_segs_neg, linewidths=0.8, alpha=0.35, colors=[(0.55, 0.55, 0.55, 1.0)], linestyles='--')
            self.ax.add_collection(lc_pos)
            self.ax.add_collection(lc_neg)
        # Depot
...
if __name__ == "__main__":
    ...
    # Lane debug toggles
    parser.add_argument("--lanes", action="store_true", help="Enable lane offsets in the viz run")
    parser.add_argument("--traffic-side", choices=["right","left"], default=None,
                        help="Override traffic side for lanes: right (default) or left")
    ...
    if args.freeze_dqn:
        ...
        cfg["EPS_DECAY"] = 1.0
    if args.lanes:
        cfg["ENABLE_LANES"] = True
    if args.traffic_side:
        cfg["TRAFFIC_SIDE"] = args.traffic_side
